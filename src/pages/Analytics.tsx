...
const Analytics: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (dateRange.startDate) filtered = filtered.filter(t => t.trade_date >= dateRange.startDate)
    if (dateRange.endDate) filtered = filtered.filter(t => t.trade_date <= dateRange.endDate)
    if (assetType) filtered = filtered.filter(t => t.asset_type === assetType)
    if (selectedTags.length > 0) {
      const tagIds = selectedTags.map(t => t.id)
      filtered = filtered.filter(trade => trade.tags?.some(({ tag_id }: any) => tagIds.includes(tag_id)))
    }
  ...
  const stats = useMemo(() => {
    ...
    return { totalTrades, totalPnL, winRate, profitFactor, averageWin, averageLoss, bestWin, worstLoss, expectancy, maxDrawdown }
  }, [filteredTrades])
      ...
      {filteredTrades.length > 0 && stats && (
        <>
          ...
          <div className="card">
            ...
            {view === 'risk' && <RiskView trades={filteredTrades} />}
...
const OverviewView: React.FC<any> = ({ trades, stats }) => {
  const sortedTrades = [...trades].sort((a, b) => new Date(a.trade_date).getTime() - new Date(b.trade_date).getTime())
  const equityData = sortedTrades.reduce((acc: any[], trade: any) => {
    const prevValue = acc.length > 0 ? acc[acc.length - 1].value : 0
    const date = new Date(trade.trade_date)
    const label = `${date.getMonth() + 1}/${date.getDate()}`

    const existingEntry = acc.find(entry => entry.date === label)
    if (existingEntry) existingEntry.value += trade.pnl || 0
    else acc.push({ date: label, value: prevValue + (trade.pnl || 0) })
    return acc
  }, [])

  const wins = trades.filter((t : Trade)=> (t.pnl || 0) > 0).length
  const losses = trades.filter((t : Trade)=> (t.pnl || 0) < 0).length
  const breakeven = trades.filter((t : Trade)=> t.pnl === 0).length
...
const PerformanceView: React.FC<any> = ({ trades, stats }) => {
  return (
    <div className="space-y-6">
